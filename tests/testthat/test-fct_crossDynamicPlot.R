...
test_that("Erreur si n_categ hors bornes", {
  expect_error(fct_crossDynamicPlot(df, "var_x", n_categ = 1), "catégories doit être entre 2 et 6")
  expect_error(fct_crossDynamicPlot(df, "var_x", n_categ = 7), "catégories doit être entre 2 et 6")
})

test_that("Erreur si variable sans label", {
  expect_error(fct_crossDynamicPlot(df, x_var = "var_sans_label"))
})

test_that("Erreur si toutes les valeurs sont NA et show_NA = FALSE", {
  expect_error(fct_crossDynamicPlot(df_na, "varInexist", show_NA = FALSE))
})

test_that("Retourne un objet ggplot", {
  p <- fct_crossDynamicPlot(df, "var_x")
...
test_that("Type de graphique : boxplot si x est discret et y est continu", {
  p <- fct_crossDynamicPlot(tab = df, x_var = "var_x", y_var = "var_y")
  expect_true("GeomBoxplot" %in% class(p$layers[[1]]$geom))
})

test_that("Facettage fonctionne", {
  p <- fct_crossDynamicPlot(df, "var_x", facet_x = "facet1", facet_y = "facet2")
